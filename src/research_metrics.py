...
def daily_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = frame[(frame["minute_of_day"] >= 9 * 60 + 15) & (frame["minute_of_day"] <= 15 * 60 + 30)]
    ...
    for trade_date, day in frame.groupby("trade_date", sort=True):
        ...
        post10 = day.iloc[10:]
        post10_up = (post10["high"].max() / close10 - 1.0) if close10 else 0.0
        post10_down = (post10["low"].min() / close10 - 1.0) if close10 else 0.0
        ...
        # A deliberately descriptive, not yet weighted, reversal flag.
        # Final qualification thresholds will be learned/validated later.
        reversal = 0
